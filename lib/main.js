  ...
  start () {
    return this.resolveOptions()
      ...
      .tap(options => {
        ...
        primaryModule.on('log', data => {
          try {
            const record = JSON.parse(data)
            process.stdout.write(`[${red('main')}] ${record.message}\n`)
          } catch (e) {
            process.stdout.write(`[${bold('devl')}] can't parse log from main module\n`)
            process.stdout.write(`[${red('main')}] ${data}\n`)
          }
        })
