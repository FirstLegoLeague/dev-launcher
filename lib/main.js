...
exports.Main = class Main {
  ...
  start () {
    return this.resolveOptions()
      .tap(options => {
        const mhubService = new MhubService(options, this._logger)
        this._services.push(mhubService)

        mhubService.on('exit', code => {
          if (code) {
            this._logger.info(`MHub closed with code: ${code}`)
          } else {
            this._logger.info(`MHub closed`)
          }
      ...
      .tap(options => {
        const configurator = new Configurator(options, this._logger)
        this._services.push(configurator)

        return configurator.start()
      })
      .tap(options => {
        const primaryModule = new PrimaryModule(options, this._logger)
        this._services.push(primaryModule)

        primaryModule.on('exit', () => {
          this._logger.info(`Main module closed`)
          this.exit()
        })

        this._logger.info(`Starting primary command: ${options.moduleCommand} ` +
          options.moduleArguments.map(x => `"${x}"`).join(' '))
