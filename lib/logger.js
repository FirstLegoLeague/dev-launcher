...
const devlFormat = options => {
  const maybeApplyColors = options.noColor ? s => s : applyColors

  const mapServiceName = getServiceMap((name, service) => maybeApplyColors(service.colors, name))
  const formatMessage = r => maybeApplyColors(logLevels[r.level].colors, r.message.split('\n').join('\n\t'))
  const formatLevel = r => maybeApplyColors(logLevels[r.level].colors, r.level[0].toUpperCase())
